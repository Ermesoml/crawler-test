const { UrlsAnalyser } = require("./index.js");
...
describe("UrlsAnalyser", () => {
  ...
  beforeEach(() => {
    ...
    MemoryQueue.mockImplementation(() => ({
      push: queuePushMock,
    }));
  ...
  it("should check URL and return status", async () => {
    const urlProperties = { domain: "example.com", path: "/test" };
    const mockResponse = { status: 200 };
    fetchMock.mockResolvedValue(mockResponse);

    const result = await analyser.urlCheckerTask(urlProperties);
  ...
  it("should analyze domains and paths", async () => {
    ...
    queuePushMock.mockImplementation(async (urlProperties) => {
      return analyser.urlCheckerTask(urlProperties)
    });
